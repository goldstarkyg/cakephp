...
Class GymMemberController extends AppController
{
	...
	{	
		...
		if($this->request->is("post"))
		{
			...
			{
				$this->request->data['membership_valid_from'] = date("Y-m-d",strtotime($this->request->data['membership_valid_to']));				
			}else{
                $this->request->data['membership_valid_from'] = date("Y-m-d");
			}
			if(isset($this->request->data['membership_valid_to']))
			{
				...
				$this->request->data['membership_valid_to'] = date("Y-m-d");
			}
	...
	public function printCard()
	{
		...
		$sys_data = $this->GymMember->find("all")->select(["image","card_image","first_name","last_name","gender","birth_date","address"])->where(["id"=>$id])->first();
				
		if($sys_data['card_image'] == '' || $sys_data['card_image'] == null)
			$sys_data['card_image'] = 'card-image.png';
