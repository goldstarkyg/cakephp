	...
	public function saveSetting()
	{
		...
		if($this->request->is("post"))
		{
			...
			{   
				...
				$logo_name =  $this->GYMFunction->uploadLogoImage($this->request->data['gym_logo']);
				$cover_name =  $this->GYMFunction->uploadImage($this->request->data['cover_image']);				
                $this->request->data['gym_logo'] = $logo_name;// 'logo.'.$logo_name_buffer[1];
				...
				if($this->request->data['gym_logo']['name'] != "")
				{
					...
					$session->write("User.logo","/webroot/upload/".$logo_name);
				}else{
                    $this->request->data['gym_logo'] = $update_row['gym_logo'];
				}
				if($this->request->data['cover_image']['name'] != "")
				{
					$this->request->data['cover_image'] = $cover_name;
				}else{
                    $this->request->data['cover_image'] = $update_row['cover_image'];
				}
