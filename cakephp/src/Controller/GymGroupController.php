...
Class GymGroupController extends AppController
{
	...
	public function deleteGroup($id = null)
	{
		if($id != null)
		{
            ...
            $member = TableRegistry::get("GymMember");
            $member_group = $member->find("all")->select(["id","assign_group"])->hydrate(false)->toArray();
            
            //var_dump($member_group);return;
            $v = array();
            foreach ($member_group as $key=>$value) {
				...
				//if($group["assign_group"] != null && $group["assign_group"] != "")
