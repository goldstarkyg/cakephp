		...
		<div class="box-body">
			...
			foreach($data as $row)
			{
				echo "<tr>
					...
					<td>
						<a href='".$this->request->base ."/GymNutrition/viewNutrition/{$row['user_id']}' class='btn btn-primary btn-flat'><i class='fa fa-eye'></i> ".__("View Nutrition")."</a>";
                if($row['user_id'] != 0)
				echo "<a href='{$this->request->base}/GymNutrition/deleteNutrition/{$row['id']}' title='Delete' style='margin-left:5px' class='btn btn-flat btn-danger' onClick=\"return confirm('Are you sure,You want to delete this nutrition?');\"><i class='fa fa-trash-o'></i></a>
					</td>
				</tr>";
			}
			?>
