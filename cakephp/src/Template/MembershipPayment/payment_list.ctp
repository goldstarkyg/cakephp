...
$(document).ready(function() {
	...
	jQuery('#payment_listAdmin').DataTable({
	"responsive": true,
	"order": [[ 0, "asc" ]],
	"aoColumns":[
				  {"bSortable": true},
...
<section class="content">
	...
	<div class="col-md-12 box box-default">		
		...
		<hr>
		<div class="box-body">
		<table id="payment_listAdmin" class="table table-striped" cellspacing="0" width="100%">
        	<thead>
            <tr>
		...
		?>
		<h2>
			<small><?php echo __("Payment History");?></small>
		</h2>
		 <table id="payment_list" class="table table-striped" cellspacing="0" width="100%">
        	<thead>
            <tr>
				<th><?php  echo __( 'PaymentDate', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Paid Amount', 'gym_mgt' ) ;?></th>
				<th><?php  echo __( 'Payment Method', 'gym_mgt' ) ;?></th>
            </tr>
			</thead>
			<tbody>
				<?php				
					//echo("<script>console.log('PHP: ".print_r($tempRow)."');</script>");
					if(!empty($history_data))
					{
						foreach($history_data as $row)
						{
							echo "<tr>
							<td>".date($this->Gym->getSettings("date_format"),strtotime($row["paid_by_date"]))."</td>
							<td>{$row['amount']}</td>
